...
const ServicosCadastro: React.FC = () => {
  ...
  const servicosFiltrados = servicos.filter((s) =>
    search ? (s.SERVICO || "").toLowerCase().includes(search.toLowerCase()) : true
  );

  // Edição inline
  const handleEdit = (servico: Servico) => {
    setEditId(servico.id);
    setEditNome(servico.SERVICO);
  };

  const handleEditSave = async (id: number) => {
    const sucesso = await editarServico(id, editNome);
    if (sucesso) {
      setEditId(null);
      setEditNome("");
    }
  ...
  const handleNovo = async () => {
    const sucesso = await criarServico(novoNome);
    if (sucesso) {
      setNovoNome("");
      setShowNovo(false);
    }
                        ...
                        {editId === servico.id ? (
                          <>
                            <button
                              ...
                              onClick={() => handleEditSave(servico.id)}
                              title="Salvar"
                              disabled={loading}
                            >
                              ✔
