...
const ClienteEdicao: React.FC = () => {
  ...
  const id = searchParams.get("id"); // Extrai o 'id' da query string

  // Carregar dados do cliente
  useEffect(() => {
    if (!id) {
      setSubmitError("ID do cliente não fornecido na URL.");
      setFetching(false);
      return;
    }
    ...
    const fetchCliente = async () => {
      try {
        ...
        // Aplica máscaras aos dados recebidos
        const maskedData: FormData = {
          nome: cliente.NOME || "",
          email: cliente.EMAIL || "",
          telefone: cliente.TELEFONE
            ? cliente.TELEFONE.replace(/(\d{2})(\d{4,5})(\d{4})/, "($1) $2-$3")
            : "",
          cpf: cliente.CPF
        ...
        setSubmitError("Erro ao carregar dados do cliente. Tente novamente.");
        setFetching(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!id) {
      setSubmitError("ID do cliente não fornecido.");
      return;
    }
    ...
    // Remove caracteres de formatação e garante que DATA_NASCIMENTO seja apenas data
    const cleanPayload = {
      NOME: form.nome,
    ...
    try {
      await api.put(`/clientes/${id}/`, cleanPayload);
      alert("Cliente atualizado com sucesso!");
      navigate("/clientes");
    } catch (error) {
      console.error("Erro ao atualizar cliente:", error);
      setSubmitError("Erro ao atualizar cliente. Verifique os dados e tente novamente.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex flex-col sm:flex-row justify-start gap-3 sm:gap-4 mt-4 sm:mt-6">
          <button
            ...
            className={`px-4 sm:px-6 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition w-full sm:w-auto ${
              loading || fetching ? "opacity-50 cursor-not-allowed" : ""
            }`}
