...
const EstiloList: React.FC = () => {
  ...
  // Função para buscar estilos da API
  ...
  // Buscar estilos quando página, itens por página ou busca mudar
  useEffect(() => {
    fetchEstilos();
  }, [paginaAtual, itensPorPagina, search]);

  // Edição inline
  const handleEdit = (estilo: Estilo) => {
    setEditId(estilo.id);
  ...
  const handleEditSave = async (id: number) => {
    if (!editNome.trim()) {
      setMensagem("O nome do estilo não pode estar vazio!");
      setMensagemTipo("erro");
      setTimeout(() => {
        setMensagem("");
        setMensagemTipo("");
    ...
    try {
      await api.put(`/estilo/${id}/`, {
        nome: editNome,
      });
      setEstilos((prev) =>
        prev.map((e) => (e.id === id ? { ...e, nome: editNome } : e))
      );
      setEditId(null);
      ...
      setMensagem("Erro ao atualizar estilo!");
      setMensagemTipo("erro");
      setTimeout(() => {
        setMensagem("");
        setMensagemTipo("");
  ...
  const handleNovo = async () => {
    ...
    try {
      const response = await api.post("/estilo/", {
        nome: novoNome,
      });
      setEstilos((prev) => [...prev, response.data]);
      setNovoNome("");
      setShowNovo(false);
      ...
      setMensagem("Erro ao cadastrar estilo!");
      setMensagemTipo("erro");
      setTimeout(() => {
        setMensagem("");
        setMensagemTipo("");
  ...
  const confirmDelete = async () => {
    try {
      await api.delete(`/estilo/${deleteId}/`);
      setEstilos((prev) => prev.filter((e) => e.id !== deleteId));
      setDeleteId(null);
      ...
      setMensagem("Erro ao excluir estilo!");
      setMensagemTipo("erro");
      setTimeout(() => {
        setMensagem("");
        setMensagemTipo("");
            ...
            <tbody>
              {loading ? (
                ...
                estilos.map((estilo) => (
                  ...
                  >
                    <td className="px-4 py-2 border-b">{estilo.id}</td>
                      ...
                      {editId === estilo.id ? (
                        <>
                          <Button
                            ...
                            onClick={() => handleEditSave(estilo.id)}
                            title="Salvar"
                          >
                            <FaCheck />
                          </Button>
          ...
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
            <div className="bg-white dark:bg-gray-800 rounded shadow-lg p-6 w-full max-w-sm">
              ...
              <p className="mb-4 text-gray-700 dark:text-gray-300">
                Deseja realmente excluir este estilo?
              </p>
